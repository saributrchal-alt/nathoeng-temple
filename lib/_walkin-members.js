...
export async function handleWalkinMemberRequest(req, res) {
  ...
  if (action === 'member_card_number') {
    ...
    try {
      ...
      const [member] = await db(url, key, `/rest/v1/members?id=eq.${encodeURIComponent(memberId)}&select=*&limit=1`);
      return res.status(200).json({ success: true, cardNumber, idCardVerifiedAt: member?.id_card_verified_at || null });
    } catch (error) {
