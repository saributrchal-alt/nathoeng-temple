...
import { memberIdFromNumber } from '../src/memberNumber.js';
...
export async function handleWalkinMemberRequest(req, res) {
  ...
  if (!['login', 'lookup', 'lookup_member_number', 'register', 'change_password', 'admin_member', 'admin_edit_member', 'start_as_member', 'stop_as_member'].includes(action))
  ...
  if (action === 'lookup_member_number') {
    const memberId = memberIdFromNumber(req.body?.memberNumber);
    if (!memberId) return res.status(400).json({ success: false, message: 'Enter a valid 39-digit member number' });
    try {
      const rows = await db(url, key, `/rest/v1/members?id=eq.${memberId}&select=*&limit=1`);
      const member = rows?.[0];
