...
export async function handleWalkinMemberRequest(req, res) {
  ...
  if (action === 'admin_member' || action === 'admin_edit_member') {
    ...
    try {
      ...
      await db(url, key, '/rest/v1/rpc/' + rpcPath, 'POST', {
      ...
      if (error.code === 'PGRST202' || error.code === 'PGRST205')
        return res.status(503).json({ success: false, message: 'Run supabase/member-address-fields.sql in Supabase SQL Editor' });
