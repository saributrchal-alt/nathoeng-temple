...
import { isValidShortMemberNumber, memberIdFromNumber } from '../src/memberNumber.js';
...
export async function handleWalkinMemberRequest(req, res) {
  ...
  if (action === 'lookup_member_number') {
    ...
    if (!legacyId && !isValidShortMemberNumber(number))
      return res.status(400).json({ success: false, message: 'Enter a valid 13- or 39-digit member number' });
    try {
      const cardRows = legacyId ? null : await db(url, key,
        `/rest/v1/member_card_numbers?card_number=eq.${number}&select=member_id&limit=1`);
