...
export async function handleWalkinMemberRequest(req, res) {
  ...
  if (!['login', 'lookup', 'register', 'change_password', 'admin_member', 'admin_edit_member', 'start_as_member', 'stop_as_member'].includes(action))
  ...
  if (action === 'start_as_member') {
    ...
    if (!memberId || memberId.length > 100 || /[\s,()]/.test(memberId) || memberId === String(session.memberId))
    ...
    try {
      ...
      const member = members?.[0];
      if (!member || member.role !== 'member') return res.status(400).json({ success: false, message: 'Choose a regular member account' });
