...
export async function handleWalkinMemberRequest(req, res) {
  ...
  if (action === 'admin_member' || action === 'admin_edit_member') {
    ...
    const memberAddress = String(req.body?.memberAddress || '').trim();
    ...
    try {
      ...
      await db(url, key, details.available ? '/rest/v1/rpc/admin_edit_member_with_details' : '/rest/v1/rpc/admin_edit_member', 'POST', {
      ...
      return res.status(200).json({ success: true, member: {
        id: memberId, full_name: fullName, full_name_en: fullNameEn, member_address: memberAddress,
      ...
      if (error.code === 'PGRST202' || error.code === 'PGRST205')
        return res.status(503).json({ success: false, message: 'Run supabase/member-profile-details.sql in Supabase SQL Editor' });
